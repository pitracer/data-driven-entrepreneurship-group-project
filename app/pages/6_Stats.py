...
k3.metric("Firms with full Orbis data", int(df_full[feature_cols].dropna().shape[0]))
...
features_used = feature_cols if use_equity else [c for c in feature_cols if c != "orbis_equity_ratio_latest"]
...
try:
    ...
    results_df = results_df[results_df["Feature"] != "(intercept)"].reset_index(drop=True)

    # ── Results table ─────────────────────────────────────────────────────────
    sig_flags  = results_df["Significant"].tolist()
    display_df = results_df.drop(columns="Significant")

    def style_row(row):
        color = "rgba(255,179,0,0.15)" if sig_flags[row.name] else ""
    ...
    plot_df = results_df.sort_values("Coef (std)").reset_index(drop=True)
    ...
    fig_coef.update_layout(
        **CHART_LAYOUT,
        height=300,
        xaxis_title="Standardised coefficient (positive = increases scaling odds)",
        yaxis=dict(automargin=True),
    )
    st.plotly_chart(fig_coef, use_container_width=True)

    # Key findings callout
    sig_pos = results_df[results_df["Significant"] & (results_df["Coef (std)"] > 0)]
    sig_neg = results_df[results_df["Significant"] & (results_df["Coef (std)"] < 0)]
    findings = (
        [f"<b>{r['Feature']}</b> increases scaling odds by {(r['Odds Ratio']-1)*100:.0f}% (p={r['p-value']:.3f})"
         for _, r in sig_pos.iterrows()] +
        [f"<b>{r['Feature']}</b> decreases scaling odds (OR={r['Odds Ratio']:.2f}, p={r['p-value']:.3f})"
...
st.markdown('<div class="section-label">Distribution Comparison — Growth vs Non-Growth</div>',
            unsafe_allow_html=True)
...
try:
    ...
    sig_str = "statistically significant" if p < 0.05 else "not significant"
    st.caption(
        f"Mann-Whitney U test: p={p:.4f} — difference between groups is "
        f"{sig_str} (a=0.05) — Growth n={len(g1)}, Non-growth n={len(g0)}"
    )
except ImportError:
    pass
